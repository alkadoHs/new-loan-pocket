...
public function create_customer() {
    $this->form_validation->set_rules('comp_id', 'company', 'required');
    $this->form_validation->set_rules('blanch_id', 'blanch', 'required');
    $this->form_validation->set_rules('empl_id', 'blanch', 'required');
    ...
    public function create_sponser($customer_id,$comp_id){
      ...
      VALUES ('".$sp_name[$i]."','".$sp_mname[$i]."','".$sp_lname[$i]."','".$sp_phone_no[$i]."','".$sp_nation[$i]."','".$nature[$i]."','".$sp_region[$i]."','".$sp_relation[$i]."','".$sp_district[$i]."','".$sp_ward[$i]."','".$sp_street[$i]."','".$comp_id[$i]."','".$customer_id[$i]."')");
       ...
       $this->session->set_flashdata('massage','Sponsers saved successfully');        
        ...
        return redirect("admin/loan_applicationForm/".$customerdata);        
    }
	...
	public function create_loanapplication($customer_id) {
		...
		if ($this->form_validation->run()) {
			...
			if ($how_loan < $loan_price) {
				...
				// Add created_by user ID from session
  ...
  $query = $this->db->query($sqldata);
   return true;
	...
	public function aprove_loan($loan_id) {
		...
		// Update loan status
		$update_status = $this->queries->update_status($loan_id, $data);
	
		if ($update_status) {
			// Get loan and customer details
			$loan_details = $this->queries->get_loanDisbarsed($loan_id); // Fetch all loan details
			if ($loan_details) {
				// Extract necessary data
				$loan_aprove = $loan_details->loan_aprove; // Approved loan amount
				$comp_name = $loan_details->comp_name; // Company name
				$comp_phone = $loan_details->comp_phone; // Company phone number
				$phone = $loan_details->phone_no; // Customer phone number
